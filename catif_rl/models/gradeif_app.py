...
from diffusion.utils import PredefinedNoiseScheduleDiscrete  
from diffusion.model.egnn_pytorch.egnn_pytorch_geometric import EGNN_Sparse  
from diffusion.model.egnn_pytorch.utils import nodeEncoder, edgeEncoder  
...
class Trianer(object):
    def __init__(
        ...
        *,
        ...
        dl = DataLoader(self.ds, batch_size = train_batch_size, shuffle = True, pin_memory = True, num_workers = 12)
...
if __name__ == "__main__" :
    ...
    if config['dataset'] == 'CATH':
        ...
        train_ID = [f for f in os.listdir(config['train_dir']) if f.endswith('.pt')]
        val_ID   = [f for f in os.listdir(config['val_dir'])   if f.endswith('.pt')]
        test_ID  = [f for f in os.listdir(config['test_dir'])  if f.endswith('.pt')]
