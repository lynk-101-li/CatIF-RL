...
MASTER = HERE / "master_per_protein.csv"
...
table_S7.to_csv(HERE / "table_S7_full_baseline.csv", index=False)
(HERE / "table_S7_full_baseline.md").write_text(df_to_md(table_S7,
...
print(f"  wrote {HERE/'table_S7_full_baseline.csv'}")
...
table_S8.to_csv(HERE / "table_S8_round_wise.csv", index=False)
(HERE / "table_S8_round_wise.md").write_text(df_to_md(table_S8,
    "Table S8. CatIF and the three GRPO rounds — per-protein metrics with 95 % CI and joint success rate at multiple δ."))
print(f"  wrote {HERE/'table_S8_round_wise.csv'}")
...
table_S9.to_csv(HERE / "table_S9_threshold_sensitivity.csv", index=False)
(HERE / "table_S9_threshold_sensitivity.md").write_text(df_to_md(table_S9,
    "Table S9. Joint success rate sensitivity to the Δlog10 k_cat threshold δ "
    "(pLDDT > 90 and backbone RMSD < 4 Å held fixed)."))
print(f"  wrote {HERE/'table_S9_threshold_sensitivity.csv'}")
...
table_S10.to_csv(HERE / "table_S10_ablation_pairwise.csv", index=False)
(HERE / "table_S10_ablation_pairwise.md").write_text(df_to_md(table_S10,
...
print(f"  wrote {HERE/'table_S10_ablation_pairwise.csv'}")
